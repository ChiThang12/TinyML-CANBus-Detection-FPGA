...
def clean_mem_file(input_file, output_file):
    """Remove comments and empty lines from .mem file"""
    with open(input_file, "r") as f_in, open(output_file, "w") as f_out:
        for line in f_in:
            clean_line = line.split("//")[0].strip()
            if clean_line:
                f_out.write(clean_line + "\n")
    print(f"   ✔ Cleaned MEM → {output_file}")
...
def main():
    ...
    NUM_TREES = 21   # tree_0 → tree_20

    for i in range(NUM_TREES):
        csv_file = f"LUT/tree_{i}_v.csv"
        mem_output = f"./SW/MEM/tree_{i}_demo.mem"
        mem_clean = f"./SW/MEM/tree_{i}.mem"

        print(f"\n🔄 Processing tree {i}...")
        print(f"   Input CSV:  {csv_file}")
        print(f"   Output MEM: {mem_output}")

        # Create converter
        converter = TreeToBinaryMemConverter(csv_file)

        # Generate MEM
        binary_values = converter.generate_binary_mem_file(mem_output)

        # Verify correctness
        verify_conversion(csv_file, binary_values, converter)

        # Clean MEM file (remove comments)
        clean_mem_file(mem_output, mem_clean)

    print("\n" + "=" * 70)
    print("✅ ALL TREE MEM FILES GENERATED AND CLEANED!")
    print("=" * 70)

    print("\n📦 Output includes:")
    print("  tree_0.mem ... tree_20.mem           - raw MEM files")
    print("  tree_0_clean.mem ... tree_20_clean.mem - cleaned MEM files (for FPGA)")
